...
import com.aiqfome.aiqcomponents.progressbutton.AllCapsSpannedTransformationMethod
...
internal fun TextView.showProgress(params: ProgressParams) {
    params.apply {
        ...
        val progressStrokeValue = progressStrokeRes?.let { res.getDimensionPixelSize(it) }
                ?: progressStrokePx
        val progressRadiusValue = progressRadiusRes?.let { res.getDimensionPixelSize(it) }
                ?: progressRadiusPx
        val colors = when {
